//
//  LocationMath.sharedInstance.swift
//  ARProject00
//
...
class LocationMath {
    ...
    static let sharedInstance = LocationMath()
    
    //one meter ~ coordinate points
    func metersToCoordinate(metr: Double) -> Float {
        return Float(metr*DEFAULT_METR_SCALE)
    }
  
    //convert LL to XY having center (0,0) in specific LL location
    func convertLLtoXY(origin: CLLocation, newLocation: CLLocation) -> Point2D {
...
//        let point = Point2D(xPos: x * DEFAULT_METR_SCALE, yPos: y * DEFAULT_METR_SCALE)
    ...
    func meterDegLon(x: Double) -> Double {
    ...
    func meterDegLat(x: Double) -> Double {
        let d2r = DegreesToRadians(x)
    ...
    func covertToMeters(feet: Double) -> Double {
        return feet/FEET
    }
    
    func isPointLeft(a: Point2D, b: Point2D, c: Point2D) -> Bool {
        let value: Double = ((b.x - a.x)*(c.y - a.y) - (b.y - a.y)*(c.x - a.x))
        
        return value < 0
    }
    
    func angleBetween2Lines(line1: Line2D, line2: Line2D) -> Double {
    ...
    func angleBetween2DotsWithCenter(centerPoint: Point2D, point1: Point2D, point2: Point2D) -> Double {
